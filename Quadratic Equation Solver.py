...
def turning_point(x1, x2, a, b, c):
  x = (x1 + x2) / 2
  y = (a * (x ** 2)) + (b * x) + c
  return f"({convert(x)}, {convert(y)})"
...
class Equation:
  ...
  def Solve(self):
    ...
    try:
      ...
      if self.x1 != self.x2:
        print(f"\nThe roots of the quadratic (the x-intercepts) are {convert(self.x1)} and {convert(self.x2)}.\n")
      else:
      ...
      print(f" - The turning point of the parabola is {turning_point(self.x1,self.x2,self.a,self.b,self.c)}.")
